...
TRANSLATIONS = {
    ...
    "cm.caption": {
        "ru": "Все площадки в одной картине: продажи, цены, паритет, остатки, здоровье каналов",
        "uk": "Усі майданчики в одній картині: продажі, ціни, паритет, залишки, здоров’я каналів",
        "en": "Every channel in one picture: sales, prices, parity, stock, channel health",
    },
    "cm.intro.title": {
    ...
    "cm.tab.parity": {"ru": "Паритет цен", "uk": "Паритет цін", "en": "Price parity"},
    "cm.par.no_data": {"ru": "Данные о паритете цен не загрузились. Что ответила база: {e}. Если таблицы нет в схеме kabinet_data — её не создали или не досинхронизировали из Unity Catalog.", "uk": "Дані про паритет цін не завантажилися. Що відповіла база: {e}. Якщо таблиці немає у схемі kabinet_data — її не створили або не досинхронізували з Unity Catalog.", "en": "Price parity data did not load. What the database answered: {e}. If the table is absent from the kabinet_data schema, it was never created or never synced from Unity Catalog."},
    "cm.par.no_columns": {
        "ru": "В таблице price_parity нет колонок: {miss}. Страница ждёт их по договорённости со сборщиком и не угадывает имена. Что в таблице есть: {have}.",
        "uk": "У таблиці price_parity немає колонок: {miss}. Сторінка чекає на них за домовленістю зі збирачем і не вгадує імена. Що в таблиці є: {have}.",
        "en": "Table price_parity is missing columns: {miss}. The page expects them by agreement with the loader and does not guess names. What the table does have: {have}.",
    },
    "cm.par.kpi_alert": {"ru": "Разброс больше {n} %", "uk": "Розкид більший за {n} %", "en": "Spread over {n} %"},
    "cm.par.kpi_alert_help": {"ru": "Товары, где самая высокая цена отличается от самой низкой больше чем на порог. Считается только по каналам, где цена известна; товар с ценой на одном канале в счёт не идёт.", "uk": "Товари, де найвища ціна відрізняється від найнижчої більше ніж на поріг. Рахується лише за каналами, де ціна відома; товар із ціною на одному каналі не враховується.", "en": "Products where the highest price differs from the lowest by more than the threshold. Counted only across channels with a known price; a product priced on one channel is not counted."},
    "cm.par.kpi_bad": {"ru": "Больше {n} %", "uk": "Більше за {n} %", "en": "Over {n} %"},
    "cm.par.kpi_double": {"ru": "Дороже вдвое и более", "uk": "Дорожче вдвічі й більше", "en": "Twice the price or more"},
    "cm.par.kpi_double_help": {"ru": "Разброс больше 100 % значит, что где-то товар стоит как минимум вдвое дороже, чем на самом дешёвом канале. Это уже не разница в комиссиях.", "uk": "Розкид більший за 100 % означає, що десь товар коштує щонайменше вдвічі дорожче, ніж на найдешевшому каналі. Це вже не різниця в комісіях.", "en": "A spread above 100 % means the product costs at least twice as much somewhere as on the cheapest channel. That is no longer a difference in fees."},
    ...
    "cm.par.note": {
        ...
        "en": "The deviation is computed by the loader; the page takes it as given and does not recompute it — an in-page calculation alongside someone else's will drift apart eventually. Eight euro channels take part in the comparison; the British price in pounds sits in the last column and stays out of the spread, since next to euros it would inflate it for no reason.",
    },
    ...
    "ads.act.title": {"ru": "Действие по кампании", "uk": "Дія по кампанії", "en": "Campaign action"},
    "ads.act.no_keys": {"ru": "Ключи Amazon Ads не читаются из скоупа amazon-sp-api. Кнопки появятся, когда принципалу приложения выдадут READ на скоуп: databricks secrets put-acl amazon-sp-api 583bf6d1-6cd0-4a89-9c44-b387ec5c21cb READ", "uk": "Ключі Amazon Ads не читаються зі скоупу amazon-sp-api. Кнопки зʼявляться, коли принципалу застосунку видадуть READ на скоуп: databricks secrets put-acl amazon-sp-api 583bf6d1-6cd0-4a89-9c44-b387ec5c21cb READ", "en": "Amazon Ads keys cannot be read from the amazon-sp-api scope. The buttons appear once the app principal is granted READ on the scope: databricks secrets put-acl amazon-sp-api 583bf6d1-6cd0-4a89-9c44-b387ec5c21cb READ"},
    "ads.act.no_campaigns": {"ru": "За этот период нет кампаний, по которым можно действовать: у скрытых строк Amazon не отдаёт идентификатор.", "uk": "За цей період немає кампаній, за якими можна діяти: у прихованих рядків Amazon не віддає ідентифікатор.", "en": "No actionable campaigns in this period: Amazon does not return an id for masked rows."},
    "ads.act.pick": {"ru": "Кампания", "uk": "Кампанія", "en": "Campaign"},
    "ads.act.what": {"ru": "Что сделать", "uk": "Що зробити", "en": "Action"},
    "ads.act.pause": {"ru": "поставить на паузу", "uk": "поставити на паузу", "en": "pause"},
    "ads.act.bid_down": {"ru": "снизить ставку", "uk": "знизити ставку", "en": "lower bid"},
    "ads.act.pct": {"ru": "На сколько, %", "uk": "На скільки, %", "en": "By, %"},
    "ads.act.sb_no_bid": {"ru": "В Sponsored Brands ставка задана у ключевых слов, а не у групп: отсюда доступна только пауза.", "uk": "У Sponsored Brands ставка задана в ключових слів, а не в груп: звідси доступна лише пауза.", "en": "In Sponsored Brands bids live on keywords, not ad groups: only pause is available here."},
    "ads.act.prepare": {"ru": "Подготовить", "uk": "Підготувати", "en": "Prepare"},
    "ads.act.reading": {"ru": "Читаю текущее состояние в Amazon…", "uk": "Читаю поточний стан в Amazon…", "en": "Reading the current state from Amazon…"},
    "ads.act.confirm_pause": {"ru": "**{n}** — поставить на паузу. Сейчас состояние: `{s}`.", "uk": "**{n}** — поставити на паузу. Зараз стан: `{s}`.", "en": "**{n}** — pause. Current state: `{s}`."},
    "ads.act.confirm_bid": {"ru": "**{n}** — снизить ставку на {p} % у {g} работающих групп:", "uk": "**{n}** — знизити ставку на {p} % у {g} робочих груп:", "en": "**{n}** — lower bids by {p} % in {g} enabled ad groups:"},
    "ads.act.col_group": {"ru": "Группа объявлений", "uk": "Група оголошень", "en": "Ad group"},
    "ads.act.col_before": {"ru": "Было", "uk": "Було", "en": "Before"},
    "ads.act.col_after": {"ru": "Станет", "uk": "Стане", "en": "After"},
    "ads.act.skipped": {"ru": "Выключенные группы не трогаем: {n}", "uk": "Вимкнені групи не чіпаємо: {n}", "en": "Paused ad groups are left alone: {n}"},
    "ads.act.no_groups": {"ru": "У кампании нет работающих групп со ставкой — снижать нечего.", "uk": "У кампанії немає робочих груп зі ставкою — знижувати нічого.", "en": "The campaign has no enabled ad groups with a bid — nothing to lower."},
    "ads.act.confirm": {"ru": "Подтвердить", "uk": "Підтвердити", "en": "Confirm"},
    "ads.act.cancel": {"ru": "Отмена", "uk": "Скасувати", "en": "Cancel"},
    "ads.act.done": {"ru": "Готово. Запись в журнале № {id} — оттуда же можно вернуть как было.", "uk": "Готово. Запис у журналі № {id} — звідти ж можна повернути як було.", "en": "Done. Journal entry № {id} — the rollback lives there too."},
    "ads.act.api_error": {"ru": "Amazon не ответил: {e}", "uk": "Amazon не відповів: {e}", "en": "Amazon did not answer: {e}"},
    "ads.act.api_partial": {"ru": "Amazon принял не всё ({s}). Ответ: {e}", "uk": "Amazon прийняв не все ({s}). Відповідь: {e}", "en": "Amazon accepted only part of it ({s}). Response: {e}"},
    "ads.act.actor_unknown": {"ru": "kabinet-app", "uk": "kabinet-app", "en": "kabinet-app"},
    "ads.act.log_title": {"ru": "Журнал действий ({n})", "uk": "Журнал дій ({n})", "en": "Action log ({n})"},
    "ads.act.log_empty": {"ru": "Из Кабинета пока ничего не меняли.", "uk": "З Кабінету поки нічого не змінювали.", "en": "Nothing has been changed from Kabinet yet."},
    "ads.act.log_when": {"ru": "Когда", "uk": "Коли", "en": "When"},
    "ads.act.log_who": {"ru": "Кто", "uk": "Хто", "en": "Who"},
    "ads.act.log_camp": {"ru": "Кампания", "uk": "Кампанія", "en": "Campaign"},
    "ads.act.log_what": {"ru": "Что", "uk": "Що", "en": "What"},
    "ads.act.log_was": {"ru": "Было", "uk": "Було", "en": "Before"},
    "ads.act.log_now": {"ru": "Стало", "uk": "Стало", "en": "After"},
    "ads.act.log_status": {"ru": "Ответ Amazon", "uk": "Відповідь Amazon", "en": "Amazon"},
    "ads.act.log_pause": {"ru": "пауза", "uk": "пауза", "en": "pause"},
    "ads.act.log_resume": {"ru": "включение", "uk": "увімкнення", "en": "resume"},
    "ads.act.log_bid_down": {"ru": "ставка ниже", "uk": "ставка нижче", "en": "bid lowered"},
    "ads.act.log_rollback": {"ru": "откат", "uk": "відкат", "en": "rollback"},
    "ads.act.undo": {"ru": "Вернуть как было", "uk": "Повернути як було", "en": "Roll back"},
    "ads.act.undo_pick": {"ru": "Какое действие вернуть", "uk": "Яку дію повернути", "en": "Which action to roll back"},
    "ads.act.undo_label": {"ru": "№ {id} · {c} · {a}", "uk": "№ {id} · {c} · {a}", "en": "№ {id} · {c} · {a}"},
    "ads.act.undo_none": {"ru": "Возвращать нечего: всё либо уже откачено, либо не прошло.", "uk": "Повертати нічого: усе або вже відкочено, або не пройшло.", "en": "Nothing to roll back: everything is either reverted or failed."},
    "ads.act.undo_done": {"ru": "Вернули как было. Запись № {id}.", "uk": "Повернули як було. Запис № {id}.", "en": "Rolled back. Entry № {id}."},
    "ads.act.undo_note": {"ru": "откат действия № {id}", "uk": "відкат дії № {id}", "en": "rollback of action № {id}"},
