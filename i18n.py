...
TRANSLATIONS = {
    ...
    "stock.map.out_inbound": {"ru": "Едет", "uk": "Їде", "en": "Inbound"},
    "stock.map.out_inbound_help": {"ru": "Штук в поставках, которые Amazon уже принял к приёмке: собирается, отгружено, принимается. Ноль — поставки нет вообще.", "uk": "Штук у постачаннях, які Amazon уже прийняв до приймання: збирається, відвантажено, приймається. Нуль — постачання немає взагалі.", "en": "Units in shipments Amazon has already registered: working, shipped, receiving. Zero means no shipment at all."},
    "stock.map.out_eta": {"ru": "Придёт", "uk": "Прибуде", "en": "ETA"},
    "stock.map.out_eta_help": {"ru": "Ожидаемая дата приёмки, если Amazon её сообщил. Прочерк — даты в данных нет, а не «не приедет».", "uk": "Очікувана дата приймання, якщо Amazon її повідомив. Прочерк — дати в даних немає, а не «не приїде».", "en": "Expected receiving date, when Amazon reports one. A dash means the date is missing from the data, not that nothing is coming."},
