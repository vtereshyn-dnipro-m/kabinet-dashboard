...
TRANSLATIONS = {
    ...
    "ads.act.no_keys": {"ru": "Ключи Amazon Ads не настроены: кнопки появятся, когда в секретах приложения будет раздел amazon_ads (client_id, client_secret, refresh_token, profile_id).", "uk": "Ключі Amazon Ads не налаштовані: кнопки зʼявляться, коли в секретах застосунку буде розділ amazon_ads (client_id, client_secret, refresh_token, profile_id).", "en": "Amazon Ads keys are not configured: the buttons appear once the app secrets contain an amazon_ads section (client_id, client_secret, refresh_token, profile_id)."},
