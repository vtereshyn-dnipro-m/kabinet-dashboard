...
TRANSLATIONS = {
    ...
    "cm.par.no_columns": {"ru": "В таблице не нашлось артикула и хотя бы двух колонок с ценами — сравнивать нечего. Что в ней есть: {have}.", "uk": "У таблиці не знайшлося артикула і хоча б двох колонок із цінами — порівнювати нема чого. Що в ній є: {have}.", "en": "The table has no SKU column and no two price columns — there is nothing to compare. What it does have: {have}."},
    ...
    "cm.par.note": {"ru": "В сравнение вошли колонки: {cols}. Исключено: {skip} — цена в фунтах, и рядом с евро она задрала бы разброс на ровном месте. Если в списке видите что-то, что ценой не является, скажите: колонка убирается одной строкой.", "uk": "У порівняння увійшли колонки: {cols}. Виключено: {skip} — ціна у фунтах, і поряд з євро вона задерла б розкид на рівному місці. Якщо у списку бачите щось, що ціною не є, скажіть: колонка прибирається одним рядком.", "en": "Columns included in the comparison: {cols}. Excluded: {skip} — a price in pounds, which next to euros would inflate the spread for no reason. If you see something in the list that is not a price, say so: removing a column is a one-line change."},
